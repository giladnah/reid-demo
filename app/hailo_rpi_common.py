...
def get_default_parser():
    ...
    parser.add_argument("--monitor", action="store_true", help="Dump monitor log")
    return parser
...
class GStreamerApp:
    ...
    def log_monitor(self):
        print(f"{time.time()-self.start_time} FPS {self.avg_fps}")
        return True
    
    def print_queue_fill_level(self):
        ...
        while True:
            ...
            if result != Gst.IteratorResult.OK:
                break    
            if element.get_name().startswith("debug"):
                try:
                    current_level_buffers = element.get_property("current-level-buffers")
                    print(f"{element.get_name()}: buffers={current_level_buffers}")
                except Exception as e:
                    print(f"Error retrieving stats from {element.get_name()}: {e}")
        return True
    ...
    def run(self):
        ...
        if hailo_display is None:
            ...
            if xvimagesink is not None:
                ...
                xvimagesink.set_property("max-lateness", -1)
                qos = xvimagesink.get_property("qos")
                max_lateness = xvimagesink.get_property("max-lateness")
                print(f"QoS is set to {qos}, max-lateness is set to {max_lateness}")
        ...
        if self.options_menu.monitor:
            GLib.timeout_add_seconds(30, self.log_monitor)
        
        if (True):
            GLib.timeout_add_seconds(1, self.print_queue_fill_level)
